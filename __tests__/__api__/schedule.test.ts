...
describe("予定追加ユースケース", () => {
  beforeEach(() => {
    myContainer
      .get<UserRepository>(TYPES.UserRepository)
      .clear();
    myContainer
      .get<ScheduleRepository>(TYPES.ScheduleRepository)
      .clear();
  });
  it("追加した予定が確認できること", async () => {
    ...
    expect(createdSchedule.title).toEqual(
      "詳細設計レビュー"
    );
    expect(createdSchedule.startDatetime).toEqual(
      "2024-12-11 12:00"
    );
    expect(createdSchedule.endDatetime).toEqual(
      "2024-12-11 13:00"
    );
  });
  it("予定の開始時間を終了時間と同じにしている場合、エラーが返却されること", async () => {
    ...
    expect(response.body.errorMessage).toEqual(
      "予定開始日時は予定終了日時より前に設定する必要があります startDatetime: 2024-12-11 12:00 endDatetime: 2024-12-11 12:00"
    );
  });
  it("参加者込みで予定を追加できること", async () => {
    ...
    expect(
      createdSchedule.participants[0].userName
    ).toEqual("テスト太郎");
    expect(
      createdSchedule.participants[0].participationStatus
    ).toEqual(ParticipationStatus.NOT_RESPOND);
  });
});
