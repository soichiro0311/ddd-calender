...
const scheduleRepository =
  myContainer.get<ScheduleRepository>(
    TYPES.ScheduleRepository
  );
const userRepository = myContainer.get<UserRepository>(
...
export const updateParticipationStatus = (
  ...
  try {
    const updateParticipationStatusRequest =
      new UpdateParticipationStatusRequest(request);
    respondToSchedule(
      scheduleRepository,
      userRepository,
...
export const allSchedules = (_: any, response: any) => {
  listSchdule(scheduleRepository).then((allSchedules) => {
    const scheduleDto = allSchedules.map(
      (schedule) => new ScheduleDto(schedule)
    );
